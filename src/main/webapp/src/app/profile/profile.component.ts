...
toggleView(x){
 ...
 refus(id){
  ...
  //
  //  getFiles(): Observable {
  //    return self.http.get('/getallfiles');
  //  }
   onImageChange(e){
    ...
    let self = this;
      if(e.target.files && e.target.files.length) {
       ...
       // const formdata: FormData = new FormData();
        ...
        self.startup.usrImg(self.loggedUser.id,'USER',self.file).subscribe(response =>{
          if(response.retCode != -1){
            console.log('Image uploaded successfully');
            console.log(response);
            self.sanitizeImageUrl(response);
          // if (self.loggedUser.image != null) {
          //   self.loggedUser.image.origin = response.origin;
          // }
          // self.source = self.sanitizeImageUrl(response.origin);
          //Make a call to the Spring Boot Application to save the image
          // var block = response.origin.split(";");
          // Get the content type of the image
          // var contentType = block[0].split(":")[1];// In this case "image/jpeg"
          // get the real base64 content of the file
          // var realData = block[1].split(",")[1];// In this case "R0lGODlhPQBEAPeoAJosM...."

          // to decode base64 we use atob() to encode we use btoa()
          // self.source = atob(realData);
      ...
      // let uploadImageData = new String();
      ...
      //   self.startup.usrImg(self.loggedUser.id,'USER',formdata).toPromise().then(response =>{
      //     if(response.retCode != -1){
      //       // self.notifyService.showSuccess(response.retDescription,"");
      //       console.log('Image uploaded successfully');
      //       if (self.loggedUser.image != null) {
...
refreshPage() {
  ...
  self.router.navigateByUrl('#urlabsolu', { skipLocationChange: false }).then(() => {
