...
public categories: { [key: string]: Object; }[] = [
  ...
  ngOnInit(): void {
    this.startup.annonceId(this.id).toPromise().then(response =>{
      ...
      this.notStar =  this.generateArrayOfNumbers(5 - parseInt(response.userInfo.rating));
      this.stars = this.generateArrayOfNumbers(parseInt(response.userInfo.rating));
      ...
      if(sessionStorage != undefined){
          ...
          if(this.annonce.userInfo.username.toUpperCase() === this.loggedUser.username.toUpperCase().toString() && !this.annonce.cancelled){
            this.show = true;
            this.readOnly = false;
          }
          this.getReservation();
  ...
  save(){
    ...
    console.log("annonce.startDate: ",this.annonce.startDate);
    ...
    if(this.annonce.startDate && this.annonce.startDate.toString().indexOf("/") != -1){
      dateD = this.dateToMills(this.annonce.startDate);
      console.log("2) dateD: ",dateD);
    }

    this.endDate = this.annonce.endDate;
    console.log("annonce.endDate: ",this.annonce.endDate);
