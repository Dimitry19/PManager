package cm.packagemanager.pmanager.ws.controller.rest.airplane;
...
import static cm.packagemanager.pmanager.constant.WSConstants.AIRPLANE_WS;


@RestController
@RequestMapping(AIRPLANE_WS)
@Api(value = "Airline-service", description = "Compagnies ")
public class AirlineController extends CommonController {
    ...
    @PostMapping(value =ADD, consumes = MediaType.APPLICATION_JSON,produces = MediaType.APPLICATION_JSON,headers = WSConstants.HEADER_ACCEPT)
    public @ResponseBody  ResponseEntity<Object> addCompany(HttpServletResponse response, HttpServletRequest request, @RequestBody @Valid AirlineDTO ac) throws Exception{
    ...
    ResponseEntity<Object> update(HttpServletResponse response, HttpServletRequest request,   @RequestBody @Valid UpdateAirlineDTO uar) throws Exception {
        ...
        try {
            ...
            AirlineVO airline = airplaneService.update(uar);
            if (airline != null) {
                airline.setRetCode(WebServiceResponseCode.OK_CODE);
            ...
            logger.error("Erreur durant l'ajournement de la compagnie " + uar.toString() + "{ }", e);
            throw e;
        } finally {
            finishOpentracingSpan();
        }
