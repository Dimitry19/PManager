...
public class MailSender {
	...
	public  MailSender(ServletContext context)
	{
	...
	public void sendMailMessage(String templateName, String messageSubject, Map<String, String> variableLabel, String emailSendTo, String username)
	{
		...
		Authenticator authenticator = new SMTPAuthenticator(adminEmail, adminPassword);
		...
		try
		{
			...
			logger.debug(String.format("Sending a message To:%s CC:%s BCC:%s", emailSendTo, emailbcc,emailbcc));

			MimeMessage message = new MimeMessage(mailSession);
			message.setFrom(new InternetAddress(adminEmail, fromName));

			InputStream emailTemplateStream =context.getResourceAsStream("/templates/"+templateName);
			...
			String EmailSendAddresses = emailSendTo;
			String EmailSendAddressesCC=emailbcc;


			String[] EmailSendAddressesArray = EmailSendAddresses.split(";");
			String[] EmailSendAddressesCCArray = EmailSendAddressesCC.split(";");
	...
	public static Map<String, String> replace(UserVO user, List<String> labels,String body,boolean isPassword){
		...
		for(String label : labels){
			...
			if(user!=null){
				...
				if(isPassword){
					variableLabel.put(label, user.getPassword());
				}
			}else{
